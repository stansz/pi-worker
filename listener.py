...
# Directories to auto-scan for AGENTS.md/README.md on startup (comma-separated)
SCAN_PATHS = os.environ.get("PI_WORKER_SCAN_PATHS", "/opt/data,/home").split(",")


def auto_discover_projects():
    """Scan SCAN_PATHS for AGENTS.md/README.md, add new projects to projects.yaml."""
    projects = load_projects()
    known_paths = {p["path"] for p in projects.values()}

    new_entries = {}
    for scan_path in SCAN_PATHS:
        scan_path = Path(os.path.expanduser(scan_path.strip()))
        ...
        for subdir in scan_path.iterdir():
            if not subdir.is_dir() or subdir.name.startswith("."):
                continue
            subdir_str = str(subdir.resolve())
            if subdir_str in known_paths:
                continue
            ...
            if agents.exists() or readme.exists():
                ...
                new_entries[subdir.name] = {
                    "path": subdir_str,
                    "docs": docs,
                    "services": [],
                }
                log("INFO", f"Auto-discovered project: {subdir.name} at {subdir_str}")

    if new_entries:
        # Merge into projects.yaml
        projects.update(new_entries)
        content = "# Auto-generated by pi-worker — edit to customize\n"
        for name, cfg in sorted(projects.items()):
            content += f"\n{name}:\n"
            content += f"  path: {cfg['path']}\n"
            content += f"  docs: [{', '.join(cfg['docs'])}]\n"
            if cfg.get("services"):
                content += f"  services: [{', '.join(cfg['services'])}]\n"
        PROJECTS_FILE.write_text(content)
        log("INFO", f"Added {len(new_entries)} auto-discovered projects")
